...
fn fetch_search_data(keyword: &str) -> Result<Vec<(String, String)>, Box<dyn Error>> {
    let mut result = vec![];
    let host_url = consts::TORRENTSIR_HOST_URL;
    ...
    for node in doc.find(Class("media-heading")) {
        ...
        let link = node.find(Name("a"))
            ...
            .attr("href")
            .unwrap();
        let base = format!("{}/bbs/", host_url);
        let base_url = Url::parse(&base).unwrap();
        let link = base_url.join(link).unwrap().as_str().to_owned();
...
mod tests {
    ...
    fn test_fetch_magnet_func() {
        let doc = Document::from(include_str!("../test_data/torrentsir_bbs.html"));
        let mut magnet = String::from("no magnet");
        for node in doc.find(Class("list-group")) {
            let temp = node.text();
            if temp.contains("magnet:?xt=urn:btih:") {
    ...
    fn test_fetch_search_data_func() {
        let mut result = vec![];
        let doc = Document::from(include_str!("../test_data/torrentsir_search.html"));
        for node in doc.find(Class("media-heading")) {
            let title = node.text().trim().to_owned();
            let link = node.find(Name("a"))
                .next()
                ...
                .attr("href")
                .unwrap();
            let base = format!("{}/bbs/", consts::TORRENTSIR_HOST_URL);
            let base_url = Url::parse(&base).unwrap();
            let link = base_url.join(link).unwrap().as_str().to_owned();
            result.push((title, link));
        }
        assert_eq!(
            ...
            "https://torrentsir31.com/bbs/board.php?bo_table=entertain&wr_id=18380"),
